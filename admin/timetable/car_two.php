    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($endDate < $currentDate) {
            ...
            if (!$updateResult) {
                ...
                logActivity('admin_logs', "System", "Name: " . $row['name'] . " Phone: " . $row['phone'] . " Training Has been Ended in Liva car TimeSlot was : " . $row['timeslots']);
                $msg[] = "Name: " . $row['name'] . " Phone: " . $row['phone'] . " Training Has been Ended in Liva car TimeSlot was : " . $row['timeslots'];
                        ...
                        <tr  id="<?php echo $row['id']; ?>">
                            ...
                            <td class="btns">
                                <?php if ($row['status'] == 'active') {
                                    echo "<a id='btn'  href='addDay.php?id=" . $row['id'] . "&car=" . $_GET['car'] . "&route=../timetable?car=liva#" . $row['id'] . "' style='background:green;' >+1 DAY</a>";
                                } ?>

                                <?php if ($row['status'] == 'active') {
                                    echo "<a id='btn'  href='subDay.php?id=" . $row['id'] . "&car=" . $_GET['car'] . "&route=../timetable?car=liva#" . $row['id'] . "' style='background:red;' >-1 DAY</a>";
                                } ?>

                                <?php if ($row['status'] == 'active') {
                                    echo "<a id='btn'  href='modify.php?id=" . $row['id'] . "&car=" . $_GET['car'] . "&route=" . urlencode("../timetable?car=liva") . "#modify'>MODIFY</a>";
                                } ?>
