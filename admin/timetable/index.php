...
<head>
    ...
    <style>
        ...
        #showTimeTableBtn,#viewCarAttendanceBtn {
            margin-top: 20px;
            background: #46abcc;
            color: #fff;
            border: none;
        ...
        #showTimeTableBtn:hover, #viewCarAttendanceBtn:hover {
            background: #3d92ae;
        }

        #showTimeTableBtn i , #viewCarAttendanceBtn i{
            font-size: 18px;
        }
...
<body>
    ...
    <section id="content">
        ...
        <main>
            ...
            <div class="time_table_box">
                ...
                <button id="viewCarAttendanceBtn" class="btn-download" style="margin-bottom: 20px;">
                    <i class='bx bx-calendar-check'></i>
                    <span class="text">View Car Attendance Today</span>
                </button>
                ...
                <dialog id="carAttendanceModal">
                    <div class="modal-content" style="padding: 20px;">
                        <button class="close" id="closeAtt" style="font-size: 28px; font-weight: bold; cursor: pointer; color: #aaa; border: none; background: none;">&times;</button>
                        <h2 style="margin-top: 0;">Car Attendance Today</h2>
                        <!-- <div style="margin-top: 20px; display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-end;">
                            <button id="downloadAttendancePDF" class="btn-download" style="background: #46abcc; color: #fff; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; display: flex; align-items: center; gap: 6px; font-size: 14px; transition: background 0.2s; flex: 1 1 auto; justify-content: center; min-width: 140px; max-width: 200px;">
                                <i class='bx bxs-file-pdf'></i>
                                Download PDF
                            </button>
                            <button id="downloadAttendanceImage" class="btn-download" style="background: #46abcc; color: #fff; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; display: flex; align-items: center; gap: 6px; font-size: 14px; transition: background 0.2s; flex: 1 1 auto; justify-content: center; min-width: 140px; max-width: 200px;">
                                <i class='bx bxs-image'></i>
                                Download Image
                            </button>
                            <button id="printAttendance" class="btn-download" style="background: #46abcc; color: #fff; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; display: flex; align-items: center; gap: 6px; font-size: 14px; transition: background 0.2s; flex: 1 1 auto; justify-content: center; min-width: 140px; max-width: 200px;">
                                <i class='bx bx-printer'></i>
                                Print
                            </button>
                        </div> -->
                ...
                <script>
                    ...
                    document.getElementById('downloadPDF').addEventListener('click', function() {
                        ...
                        html2canvas(content, options).then(canvas => {
                            ...
                            // Add title
                            pdf.setFontSize(16);
                            pdf.text('Timetable Report', pageWidth / 2, 15, {
                                align: 'center'
                            });
                ...
                <script>
                    document.getElementById('viewCarAttendanceBtn').addEventListener('click', function() {
                        const urlParams = new URLSearchParams(window.location.search);
                        const carId = urlParams.get('car');

                        if (carId) {
                            document.getElementById('carAttendanceModal').showModal();
                            $.ajax({
                                url: '../../api_ajax/get_today_car_attendance.php',
                                method: 'POST',
                                data: {
                                    car_table: carId
                                },
                                success: function(response) {
                                    ...
                                    // Assuming response is already a JSON object due to the error message
                                    const attendanceData = [...response].sort((a, b) => a.id - b.id);
                                    ...
                                    const headers = ['Time Slots', 'Name', 'Phone', 'Vehicle', 'Trainer', 'Start Date', 'End Date', 'Status', 'Attendance Status'];
                                    ...
                                    attendanceData.forEach(data => {
                                        ...
                                        if (data['Attendance Status'] === 'Present') {
                                            row.style.backgroundColor = '#ccffcc'; // Light green for present
                                        } else if (data['Attendance Status'] === 'Absent') {
                                            row.style.backgroundColor = '#ffcccc'; // Light red for absent
                                        }
                            ...
                            Swal.fire({
                                ...
                                text: 'Please select a car to view its attendance.',
                                confirmButtonColor: '#3085d6'
                            });
                        }
                    });

                    document.querySelector('#closeAtt').addEventListener('click', function() {
                        document.getElementById('carAttendanceModal').close();
                    });

                    document.getElementById('carAttendanceModal').addEventListener('click', function(event) {
                        if (event.target === this) {
                            document.getElementById('carAttendanceModal').close();
                        }
                    });
    ...
    <script>
        $(document).ready(function() {
            $('.addDaysBtnSel').on('change', function() {
                ...
                if (days) {
                    Swal.fire({
                        title: 'Add Days?',
                        text: `Are you sure you want to add ${days} days?`,
                        icon: 'question',
                        showCancelButton: true,
                        confirmButtonColor: '#3085d6',
                        cancelButtonColor: '#d33',
                                ...
                                success: function(response) {
                                    ...
                                    if (response.status === 'success') {
                                        Swal.fire({
                                            icon: 'success',
                                            title: 'Days Added',
                                            text: `Successfully added ${days} days`
                                        }).then((result) => {
                                            if (result.isConfirmed || result.isDismissed) {
                                                window.location.reload();
                                            }
                        ...
                        $(this).val(''); // Reset select after action
                    });
                }
            });
        });
