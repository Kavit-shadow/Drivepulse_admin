...
if (isset($_GET['id']) && isset($_GET['car']) && isset($_GET['days'])) {
    ...
    if ($row) {
        ...
        $newEndDate = date('Y-m-d', strtotime($end_date . "+$days days"));

        // Update the customer details in the database
        $update = "UPDATE `cust_details` SET endedAT = '$newEndDate', days = days + $days WHERE name = '$name' AND phone='$phone'";
        ...
        if (!$result) {
            ...
            logActivity('admin_logs', $_SESSION['admin_name'], array(
                "What" => "Added $days days to Customer in timetable",
                array(
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Missing required parameters: id, car, days'
    ]);
}
